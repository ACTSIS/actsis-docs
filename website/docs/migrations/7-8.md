...
5. We have removed the `normalize` package from the project as it was unmaintained. Search your project and replace:
	...
	with this:
	```scss
	@import '@actsis/frontend-libs/styles/scss/normalize';
	```
...
1. Check if the project is using the Storybook package `@actsis/storybook` because we changed the release numbers. Check the [eightshift-storybook repository's releases page](https://github.com/infinum/eightshift-storybook/releases) for reference. At the time of writing this, the current version is `1.4.1`.
...
5. Open `.storybook/preview.js`. It should look like this:
	...
	import { STORE_NAME } from '@actsis/frontend-libs/scripts/editor/store';
	import { storybookWindowObjects, storybookDefaultMocksCategories, storybookDefaultMocksColorPalette, storybookWpStyles } from '@actsis/frontend-libs/scripts/storybook';
